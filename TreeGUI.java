...
//							tm.set_state(tree);
		...
		undoButton.addActionListener(new ActionListener() {
			...
			// missing code to be filled in by you
				...
				try
            {tree_undo=tm.get_state();
            if(tree_undo==null)
            {outputPanel.clearPanel();
            tree=null;}
            else
			{outputPanel.drawTree(tree_undo);
			tree=tree_undo;}
...
class TreeMemento {
	...
	public AbsTree get_state() {
        ...
        {
		AbsTree tree_state = state.peek().clone();//we are making deep copy of tree object
